...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    /** Exercise 2 Addon, ContainsKey Method */
    public boolean containsKey(K key, int low, int high) {
        if (high < low) return false;         // no entry qualifies
        int mid = (low + high) / 2;
        int comp = compare(key, table.get(mid));
        if (comp == 0)
            return true;                            // found exact match
        else if (comp < 0)
            return containsKey(key, low, mid - 1);   // answer is left of mid (or possibly mid)
        else
            return containsKey(key, mid + 1, high);  // answer is right of mid
    }
    /** Version of containsKey that searches the entire table */
    public boolean containsKey(K key) { return containsKey(key, 0, table.size() - 1); }
